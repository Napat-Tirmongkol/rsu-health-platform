...
class SiteSettingsManager extends Component
{
    ...
    {
        $uniqueKey = 'unique:sys_site_settings,key,' . ($this->editingId ?? 'NULL') . ',id,clinic_id,0';
        ...
        if ($this->editingId) {
            SiteSetting::withoutGlobalScope(TenantScope::class)
                ...
                ->update($data);
            session()->flash('settings_msg', 'แก้ไข setting เรียบร้อย');
        } else {
            SiteSetting::withoutGlobalScope(TenantScope::class)->create($data);
            session()->flash('settings_msg', 'เพิ่ม setting เรียบร้อย');
        }
    ...
    public function delete(): void
    {
        if ($this->deletingId) {
            SiteSetting::withoutGlobalScope(TenantScope::class)
                ...
                ->delete();
            session()->flash('settings_msg', 'ลบ setting เรียบร้อย');
        }
    ...
    public function render()
    {
        ...
        return view('livewire.portal.site-settings-manager', ['settings' => $settings]);
    }
}
