        ...
        <div class="grid grid-cols-1 gap-3">
            @forelse($campaigns as $campaign)
                <button
                    ...
                    class="premium-card flex items-center gap-4 p-4 text-left transition-all {{ $selectedCampaign && $selectedCampaign->id == $campaign->id ? 'border-emerald-500 bg-emerald-50 ring-2 ring-emerald-500/20' : '' }}"
                >
                    ...
                    <div class="flex-1">
                        <h4 class="text-xs font-bold text-slate-800">{{ $campaign->title }}</h4>
                        <p class="mt-1 text-[10px] text-slate-500">{{ Str::limit($campaign->description, 50) }}</p>
                    </div>
            ...
            @empty
                <p class="rounded-xl bg-slate-50 p-4 text-xs text-slate-500">ยังไม่มีแคมเปญที่เปิดให้จอง</p>
            @endforelse
        </div>
    </div>
    ...
    @if($selectedCampaign)
        <div class="space-y-4">
            ...
            @if(empty($availableDates))
                <p class="rounded-xl border border-rose-100 bg-rose-50 p-4 text-xs text-rose-500">
