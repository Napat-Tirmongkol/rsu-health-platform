...
} catch (PDOException $e) { /* fallback */ $total_pending = $total_confirmed = $total_today = 0; }
...
} catch (PDOException $e) { die("Data Fetch Error: " . $e->getMessage()); }
...
<style>
    @keyframes sideIn { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
    @keyframes barSlideUp { from { transform: translate(-50%, 100%); opacity: 0; } to { transform: translate(-50%, 0); opacity: 1; } }
...
<div class="max-w-[1600px] mx-auto space-y-8 pb-32">
    ...
    <section class="bg-white border border-gray-100 p-5 rounded-[32px] shadow-sm flex flex-col lg:flex-row justify-between items-center gap-6">
        <div class="flex items-center gap-1.5 p-1.5 bg-gray-50 rounded-2xl overflow-x-auto no-scrollbar max-w-full">
            <button onclick="filterByStatus('all')" class="status-tab tab-active px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all">All</button>
            <button onclick="filterByStatus('booked')" class="status-tab px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 transition-all flex items-center gap-2">Pending <span class="px-2 py-0.5 bg-amber-100 text-amber-600 rounded-lg text-[10px]"><?= $total_pending ?></span></button>
            <button onclick="filterByStatus('confirmed')" class="status-tab px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 transition-all flex items-center gap-2">Confirmed <span class="px-2 py-0.5 bg-emerald-100 text-emerald-600 rounded-lg text-[10px]"><?= $total_confirmed ?></span></button>
            <button onclick="filterByStatus('cancelled')" class="status-tab px-5 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest text-gray-400 hover:text-gray-900 transition-all">Cancelled</button>
        </div>
        ...
        <div class="flex-1 w-full lg:max-w-md relative">
            ...
            <input type="text" id="globalSearch" placeholder="ค้นหาตามชื่อ, รหัส, หรือกิจกรรม..." class="w-full pl-12 pr-6 py-4 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-medium outline-none focus:ring-4 focus:ring-blue-50/50 focus:bg-white transition-all">
        </div>
    </section>
                ...
                <tbody id="bookingTbody" class="divide-y divide-gray-50">
                    ...
                    <tr class="booking-row group transition-all hover:bg-gray-50/50" 
                        ...
                        <td class="p-6 text-center">
                            <input type="checkbox" class="row-checkbox w-5 h-5 rounded-md border-gray-300 text-blue-600 focus:ring-blue-500 cursor-pointer" onchange="updateActionBar()">
                        </td>
                        <td class="p-6">
                            <div class="font-bold text-gray-900"><?= date('d F Y', strtotime($b['slot_date'])) ?></div>
                            <div class="text-xs text-blue-600 font-extrabold uppercase mt-1 tracking-tighter"><?= substr($b['start_time'],0,5) ?> - <?= substr($b['end_time'],0,5) ?></div>
                        </td>
                        <td class="p-6 cursor-pointer" onclick='openDrawer(this.closest("tr").dataset.details)'>
                            <div class="font-black text-gray-900 group-hover:text-blue-600 tracking-tight transition-colors"><?= htmlspecialchars($b['full_name']) ?></div>
                            <div class="text-[10px] text-gray-400 font-bold uppercase tracking-widest"><?= htmlspecialchars($b['student_personnel_id']) ?></div>
                        </td>
                        <td class="p-6">
                            <div class="text-sm font-bold text-gray-700 max-w-[200px] truncate"><?= htmlspecialchars($b['campaign_title']) ?></div>
                        ...
                        <td class="p-6 text-center">
                            ...
                            <?php else: ?>
                                <span class="px-4 py-1.5 bg-gray-50 text-gray-400 text-[10px] font-black uppercase rounded-full tracking-widest"><?= $b['status'] ?></span>
                            <?php endif; ?>
                        </td>
                        <td class="p-6 text-center">
...
<aside id="sideDrawer" class="fixed top-0 right-0 h-screen w-full md:w-[480px] bg-white z-[70] shadow-2xl translate-x-full hidden flex flex-col transition-all duration-300">
    <div class="p-8 border-b border-gray-100 flex justify-between items-center">
        <h3 class="text-2xl font-black text-gray-900 tracking-tight">Booking Info</h3>
        <button onclick="closeDrawer()" class="w-10 h-10 bg-gray-50 rounded-full flex items-center justify-center hover:bg-gray-100 transition-all"><i class="fa-solid fa-xmark"></i></button>
    </div>
...
<div id="actionBar" class="fixed bottom-10 left-1/2 -translate-x-1/2 z-[50] glass-card px-8 py-4 rounded-[32px] shadow-2xl border-2 border-blue-600/10 hidden translate-y-full flex items-center gap-10">
    ...
    <div class="flex items-center gap-3">
        <button onclick="bulkApprove()" class="bg-blue-600 text-white px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest shadow-xl shadow-blue-200 hover:brightness-110 transition-all active:scale-95">Approve All</button>
        <button onclick="bulkCancel()" class="bg-white border border-gray-100 text-red-500 px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:bg-red-50 transition-all active:scale-95">Cancel</button>
    </div>
</div>
