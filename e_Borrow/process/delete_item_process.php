...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $stmt_get = $pdo->prepare("SELECT status FROM med_equipment_items WHERE id = ? AND type_id = ?");
        ...
        // 2. ดำเนินการลบ
        $stmt_delete = $pdo->prepare("DELETE FROM med_equipment_items WHERE id = ?");
        ...
        if ($stmt_delete->rowCount() > 0) {
            ...
            if ($item['status'] == 'available') {
                $sql_update_type = "UPDATE med_equipment_types SET total_quantity = total_quantity - 1, available_quantity = available_quantity - 1 WHERE id = ?";
            } 
            // (ถ้าลบของที่ 'maintenance' ให้ลดแค่ total)
            else { 
                $sql_update_type = "UPDATE med_equipment_types SET total_quantity = total_quantity - 1 WHERE id = ?";
            }
